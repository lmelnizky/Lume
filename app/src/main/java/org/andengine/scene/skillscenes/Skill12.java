...
public class Skill12 extends SkillScene {
...
//                }
    ...
    public void createStones() {
        ...
        long interval = (long) (3000);
        int timeBetweenStones = (int)(1200);
        ...
        if (!this.variantUsed && age >= interval) {
            ...
            variantUsed = true;
            stoneDirection = randomGenerator.nextInt(4) + 1;
            variantRow = randomGenerator.nextInt(3);
            variantStage = 1;
            showStonesToScreen(stoneDirection, false);
    ...
    private void showStonesToScreen(int directionVariant, final boolean thornyFirst) {
        ...
        int oppDir = (directionVariant>2) ? directionVariant-2 : directionVariant+2;
        if (variantStage == 1) {
            addBall(false, directionVariant, variantRow, 1f*factor);
            addBall(false, oppDir, variantRow, 1f*factor);
        } else if (variantStage == 2) {
            int row1 = variantRow+1;
            if (row1 > 2) row1 = 0;
            int row2 = variantRow-1;
            if (row2 < 0) row2 = 2;
            addBall(true, directionVariant, row1, 2.6f*factor);
            addBall(true, directionVariant, row2, 2.6f*factor);
            addBall(true, oppDir, row1, 2.6f*factor);
            addBall(true, oppDir, row2, 2.6f*factor);
        }
    }
