...
//                }
                ...
                if (this.getX() < sideLength) {
                    ...
                    survivedStones++;
                    if (survivedStones >= 10) addToScore(10);
    ...
    private void showStonesToScreen(int directionVariant, final boolean thornyFirst) {
        ...
        float distance = 2.0f;
        addBall(true, 2, 0, 0, 0.5f*factor);
        addBall(true, 2, 1, 0, 0.5f*factor);
        addBall(false, 2, 2, 0, 0.5f*factor);

        addBall(true, 2, 0, 1*distance, 0.5f*factor);
        addBall(false, 2, 1, 1*distance, 0.5f*factor);

        addBall(true, 2, 1, 2*distance, 0.5f*factor);
        addBall(false, 2, 2, 2*distance, 0.5f*factor);

        addBall(true, 2, 0, 3.5f*distance, 0.5f*factor);
        addBall(false, 2, 1, 3.5f*distance, 0.5f*factor);
        addBall(true, 2, 2, 3.5f*distance, 0.5f*factor);

        addBall(false, 2, 0, 5f*distance, 0.5f*factor);
        addBall(true, 2, 1, 5f*distance, 0.5f*factor);
        addBall(true, 2, 2, 5f*distance, 0.5f*factor);

        addBall(true, 2, 0, 6f*distance, 0.5f*factor);

        addBall(true, 2, 2, 7f*distance, 0.5f*factor);
